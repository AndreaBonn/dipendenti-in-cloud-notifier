...
function extractTimbratureOggi() {
  ...
  const timbratureElements = document.querySelectorAll(
    '.timbratura, .sessione, [class*="timb"], [class*="punch"], tr, .row'
  );
...
function extractAssenze() {
  ...
  const widgetContainer = document.querySelector(
    'dic-dashboard-card-content, [class*="dashboard-card"]'
  );
  ...
  if (widgetContainer) {
    ...
    const assenzeElements = widgetContainer.querySelectorAll(
      'div, li, tr, [class*="item"], [class*="row"]'
    );
    ...
    assenzeElements.forEach((el) => {
      ...
      if (dateMatches) {
        ...
        dateMatches.forEach((dateStr) => {
          ...
          if (
            lowerText.includes('ferie') ||
            lowerText.includes('vacanza') ||
...
function checkTimbratura() {
  ...
  if (
    ...
    if (timbraUscitaFound) {
      ...
      log(
        '[Timbratura] Stato determinato da conteggio: ' + (isTimbrato ? 'TIMBRATO' : 'NON TIMBRATO')
      );
    } else {
      // Fallback: cerca elementi con classi specifiche
      const statusIndicators = document.querySelectorAll(
        '[class*="status"], [class*="stato"], [class*="badge"]'
      );
      statusIndicators.forEach((el) => {
        ...
        if (text.includes('entrato') || text.includes('presente') || text.includes('in corso')) {
          ...
          text.includes('uscito') ||
          text.includes('assente') ||
          text.includes('non timbrato')
        ) {
          isTimbrato = false;
        }
      });
      ...
      if (isTimbrato !== null) {
        log(
          '[Timbratura] Stato determinato da indicatori: ' +
...
function clickTimbraButton() {
  ...
  for (const button of buttons) {
    ...
    const hasKeyword = searchTerms.some(
      (term) =>
        text.includes(term) ||
        value.includes(term) ||
  ...
  if (candidateButtons.length > 0) {
    ...
    log(
      '[Timbratura] Prova ad aprire manualmente la pagina e verifica che il bottone sia visibile'
    );
    ...
    return {
      success: false,
      message:
        'Bottone timbratura non trovato. Assicurati di essere nella pagina corretta di Dipendenti in Cloud.',
    };
  }
}
